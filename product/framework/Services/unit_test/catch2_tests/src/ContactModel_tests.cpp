...
TEST_CASE( "test update contact", "[ContactModelTests]" ) 
{
    ...
    ucf::service::model::ContactModel contactModel(frameowrk);
    contactModel.setContacts({ucf::service::model::Contact{"1234","test name"}});

    contactModel.addOrUpdateContact(ucf::service::model::Contact{"1234","new test name"});
