...
{
public:
    DataPrivate(ucf::service::network::http::ByteBufferPtr data, ucf::service::network::http::UploadProgressFunction progressFunc)
        : mData(data)
        , mSizeLeft(data->size())
        , mOffset(0)
        , mProgressFunc(progressFunc)
    {}
    ucf::service::network::http::ByteBufferPtr getData() const { return mData; }
    size_t getSizeLeft() const { return mSizeLeft;}
    void setSizeLeft(size_t left) { mSizeLeft = left;}
    size_t getOffset() const { return mOffset; }
    void setOffset(size_t offset) { mOffset = offset;}
    ucf::service::network::http::UploadProgressFunction getProgressFunction() const { return mProgressFunc;}
private:
    ucf::service::network::http::ByteBufferPtr mData;
    size_t mSizeLeft;
    size_t mOffset;
    ucf::service::network::http::UploadProgressFunction mProgressFunc;
};


BufferPayloadData::BufferPayloadData(ucf::service::network::http::ByteBufferPtr data, ucf::service::network::http::UploadProgressFunction progressFunc)
...
size_t BufferPayloadData::readData(char* data, size_t size)
{
    if (auto sizeCopy = mDataPrivate->getSizeLeft(); sizeCopy > 0)
    {
        ...
        ucf::service::network::http::ByteBuffer slice = ucf::service::network::http::ByteBuffer(startIter, endIter);
...
{
public:
    DataPrivate(const std::string& filePath, ucf::service::network::http::UploadProgressFunction progressFunc)
        : mFilePath(filePath)
...
FilePayloadData::FilePayloadData(const std::string& filePath, ucf::service::network::http::UploadProgressFunction progressFunc)
...
size_t FilePayloadData::readData(char* data, size_t size)
{
    if (auto sizeCopy = mDataPrivate->getSizeLeft(); sizeCopy > 0)
    {
        ...
        ucf::service::network::http::ByteBuffer slice = ucf::service::network::http::ByteBuffer(sizeCopy, 0);
