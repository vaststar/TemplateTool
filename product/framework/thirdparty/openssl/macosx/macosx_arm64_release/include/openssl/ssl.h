...
# define SSL_DEFAULT_FIPS_CIPHER_LIST "ALL:!COMPLEMENTOFDEFAULT:!eNULL:!kRSA"
...
void SSL_set_replay_log_cb(SSL *s, void (*callback) (SSL *ssl));
...
/*
 * Common Criteria mode callback for applications to handle certificate usage exception.
 * x509:    pointer to the current x509 object in the session
 *    s:    pointer to the current session
 * exflag:  the certificate extension flag being checked
 * output:  return 1 if it is ok to bypass the flag
 *          return 0 if a bypass is not allowed
 */
typedef int (*ssl_cc_cert_exception_cb) (const X509 *x509, const SSL *s, uint32_t exflag);
...
/*
 * Setters for Common Criteria mode callback for certificate usage exception.
 */
void SSL_set_cc_cert_exception_cb(SSL *s, ssl_cc_cert_exception_cb callback);
void SSL_CTX_set_cc_cert_exception_cb(SSL_CTX *ctx, ssl_cc_cert_exception_cb callback);
