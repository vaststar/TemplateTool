...
struct fips_meth_st {
    ...
    void (*secure_mem_cb)(void *(*secure_malloc_cb)(size_t num, const char *file, int line),
                          void *(*secure_zalloc_cb)(size_t num, const char *file, int line),
                          void (*secure_free_cb)(void *ptr, const char *file, int line),
                          void (*secure_clear_free_cb)(void *ptr, size_t num, const char *file, int line));
    ...
    int (*selftest)(void);
    const char *(*module_spec_text) (void);
    BIGNUM *(*bn_get_rfc3526_prime) (BN_MODP modp);
    ...
    unsigned char *(*fips_tls13_kdf_extract)(const EVP_MD *md, const unsigned char *salt, 
                                             size_t salt_len, const unsigned char *key, 
                                             size_t key_len, unsigned char *prk, size_t *prk_len);

    unsigned char *(*fips_tls13_kdf_expand)(const EVP_MD *md, 
                                            const unsigned char *prk, size_t prk_len,
                                            const unsigned char *prefixlabel, size_t prefixlen, 
                                            const unsigned char *label, size_t labellen, 
                                            const unsigned char *data, size_t datalen,
                                            unsigned char *okm, size_t okm_len);

    int (*fips_dh_check_params)(const DH *dh, int *ret);
    int (*fips_dh_check_pub_key)(const DH *dh, const BIGNUM *pub_key, int *ret);
    void (*post_cb)(int (*post_callback)(int op, int id, int subid, void *ex));
};
