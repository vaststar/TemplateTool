...
int FIPS_run_selftest(void);

/* Enum defining the MODP needed from the FOM */
typedef enum {
    BN_MODP_2048 = 1,
    BN_MODP_3072,
    BN_MODP_4096,
...
int FIPS_set_post_cb(void);
BIGNUM *FIPS_mode_bn_get_rfc3526_prime(BN_MODP modp);
int FIPS_mode_dh_check_params(const DH *dh, int *ret);
int FIPS_mode_dh_check_pub_key(const DH *dh, const BIGNUM *pub_key, int *ret);
DH *FIPS_mode_dh_new_by_nid(int nid);
const char * FIPS_module_vertxt(void);
const char *FIPS_module_spec(void);
int FIPS_PKCS5_PBKDF2_HMAC(const char *pass, int passlen,
