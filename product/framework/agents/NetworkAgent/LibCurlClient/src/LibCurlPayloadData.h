...
public:
    BufferPayloadData(ucf::agents::network::http::ByteBufferPtr data, ucf::agents::network::http::UploadProgressFunction progressFunc);
    ~BufferPayloadData();
    virtual size_t readData(char* data, size_t size) override;
    virtual int seekData(curl_off_t offset, int origin) override;
private:
...
public:
    FilePayloadData(const std::string& filePath, ucf::agents::network::http::UploadProgressFunction progressFunc);
    ~FilePayloadData();
    virtual size_t readData(char* data, size_t size) override;
    virtual int seekData(curl_off_t offset, int origin) override;
private:
