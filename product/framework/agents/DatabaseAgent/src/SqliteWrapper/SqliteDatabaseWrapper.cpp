...
bool SqliteDatabaseWrapper::DataPrivate::openDatabase()
{
    ...
    if (mDatabase)
    {
        DBAGENT_LOG_WARN("database already open, dbname: " << mDatabaseConfig.fileName);
        return true;
    }
    ...
    if (SQLITE_OK != result)
    {
        ...
        DBAGENT_LOG_ERROR("open db failed, dbname: " << mDatabaseConfig.fileName
                          << ", error code: " << result
                          << ", error: " << errorMsg);
        if (mDatabase)
        {
    ...
    DBAGENT_LOG_INFO("open db successfully, dbname: " << mDatabaseConfig.fileName);

    // Enable WAL mode for better concurrency
    char* sqlError = nullptr;
    if (SQLITE_OK != sqlite3_exec(mDatabase, "PRAGMA journal_mode=WAL", nullptr, nullptr, &sqlError))
    {
        DBAGENT_LOG_WARN("failed to enable WAL mode: " << (sqlError ? sqlError : "unknown"));
        if (sqlError) sqlite3_free(sqlError);
    }
    else
    {
        DBAGENT_LOG_DEBUG("WAL mode enabled for: " << mDatabaseConfig.fileName);
    }
    ...
    {
        DBAGENT_LOG_WARN("failed to set synchronous mode: " << (sqlError ? sqlError : "unknown"));
        if (sqlError) sqlite3_free(sqlError);
    }
...
void SqliteDatabaseWrapper::DataPrivate::closeDatabase()
{
    ...
    if (mDatabase)
    {
        ...
        if (SQLITE_OK != result)
        {
            DBAGENT_LOG_WARN("close db warning, dbname: " << mDatabaseConfig.fileName
                             << ", error code: " << result);
        }
        else
        {
            DBAGENT_LOG_INFO("close db successfully, dbname: " << mDatabaseConfig.fileName);
        }
        mDatabase = nullptr;
    }
}
...
bool SqliteDatabaseWrapper::DataPrivate::execute(const std::string& commandStr)
{
    ...
    if (!mDatabase)
    {
        DBAGENT_LOG_ERROR("execute failed: database not open");
        return false;
    }
    ...
    if (SQLITE_OK != result)
    {
        ...
        if (sqlError) sqlite3_free(sqlError);
        DBAGENT_LOG_ERROR("execute db command failed, dbname: " << mDatabaseConfig.fileName
                          << ", result code: " << result
                          << ", error: " << errorMessage
...
bool SqliteDatabaseWrapper::DataPrivate::prepareStatement(const std::string& statement, sqlite3_stmt** ppStmt)
{
    ...
    if (!mDatabase)
    {
        DBAGENT_LOG_ERROR("prepareStatement failed: database not open");
        return false;
    }
    ...
    if (SQLITE_OK != result)
    {
        DBAGENT_LOG_ERROR("prepare statement failed, error code: " << result
...
bool SqliteDatabaseWrapper::DataPrivate::bindDBType(sqlite3_stmt* statement, const DatabaseDataValue& value, int index)
{
    ...
    if (value.isNull())
    {
        ...
        {
            DBAGENT_LOG_ERROR("db bind null failed, error code: " << result);
            return false;
        }
    }
    ...
    if (value.holdsType<DBSupportedTypes::STRING>())
    {
        ...
        {
            DBAGENT_LOG_WARN("db bind text failed, error code: " << result);
            return false;
        }
    }
    else if(value.holdsType<DBSupportedTypes::INT>())
    {
        ...
        {
            DBAGENT_LOG_WARN("db bind int failed, error code: " << result);
            return false;
        }
    }
    else if(value.holdsType<DBSupportedTypes::FLOAT>())
    {
        ...
        {
            DBAGENT_LOG_WARN("db bind float failed, error code: " << result);
            return false;
        }
    }
    else if(value.holdsType<DBSupportedTypes::BLOB>())
    {
        ...
        if (bufferData.empty())
        {
            ...
            {
                DBAGENT_LOG_WARN("db empty blob failed, error code: " << result);
                return false;
            }
        }
        else
        {
            ...
            {
                DBAGENT_LOG_WARN("db blob failed, error code: " << result);
                return false;
            }
        }
    }
    DBAGENT_LOG_WARN("db bind text failed");
    return false;
}
    ...
    while (SQLITE_ROW == sqlite3_step(statement))
    {
        ...
        for (int i = 0; i < numberOfColumns; ++i)
        {
            ...
            case SQLITE_BLOB:
            {
                ...
                {
                    DBAGENT_LOG_WARN("Blob seems to be invalid when reading back from the DB.");
            ...
            default:
            {
                DBAGENT_LOG_WARN("Invalid column type found: " << columnType);
...
{
    DBAGENT_LOG_INFO("create SqliteDatabaseWrapper");
}

SqliteDatabaseWrapper::~SqliteDatabaseWrapper()
{
    DBAGENT_LOG_INFO("delete SqliteDatabaseWrapper");
    try
    {
        close();
    }
    ...
    {
        DBAGENT_LOG_ERROR("close db exception: " << e.what());
    }
    catch(...)
    {
        DBAGENT_LOG_ERROR("close db unknown exception");
    }
}
...
{
    DBAGENT_LOG_DEBUG("insert single row into table: " << tableName << ", from: "
              << location.file_name() << '('
              << location.line() << ':'
              << location.column() << ") `"
    ...
    {
        DBAGENT_LOG_WARN("insert empty data into table: " << tableName);
        return false;
    }

    if (columns.size() != arguments.size())
    {
        DBAGENT_LOG_WARN("mismatch in columns and data, table: " << tableName);
        return false;
    }
    ...
    {
        DBAGENT_LOG_WARN("prepare statement failed, table: " << tableName);
        return false;
    }
    ...
    for (size_t i = 0; i < arguments.size(); ++i)
    {
        ...
        {
            DBAGENT_LOG_WARN("bind statement failed, table: " << tableName);
            success = false;
            break;
        }
    }
    ...
    if (success)
    {
        ...
        {
            DBAGENT_LOG_WARN("step statement failed, table: " << tableName << ", error code: " << result);
            success = false;
        }
    }
    ...
    {
        DBAGENT_LOG_WARN("finalize statement failed, table: " << tableName << ", error code: " << result);
        return false;
    }
...
{
    DBAGENT_LOG_DEBUG("batch insert into table: " << tableName
              << ", rows: " << arguments.size()
    ...
    for (const auto& argument : arguments)
    {
        ...
        {
            DBAGENT_LOG_ERROR("mismatch in columns and data, table: " << tableName);
            return false;
        }
    }
    ...
    if (!beginTransaction())
    {
        DBAGENT_LOG_ERROR("failed to begin transaction for batch insert, table: " << tableName);
        return false;
    }
    ...
    if (!mDataPrivate->prepareStatement(insertStatement, &statement))
    {
        DBAGENT_LOG_ERROR("prepare statement failed for batch insert, table: " << tableName);
        rollback();
        return false;
    }
    ...
    for (const auto& argument : arguments)
    {
        for (size_t i = 0; i < argument.size(); ++i)
        {
            ...
            {
                DBAGENT_LOG_ERROR("bind failed at row " << insertedCount << ", table: " << tableName);
                success = false;
                break;
            }
        }
        ...
        if (auto result = sqlite3_step(statement); SQLITE_OK != result && SQLITE_DONE != result)
        {
            DBAGENT_LOG_ERROR("step failed at row " << insertedCount << ", table: " << tableName << ", error: " << result);
            success = false;
            break;
        }
        ...
        {
            DBAGENT_LOG_ERROR("reset failed at row " << insertedCount << ", table: " << tableName << ", error: " << result);
            success = false;
            break;
        }
    ...
    if (success)
    {
        if (commit())
        {
            DBAGENT_LOG_INFO("batch insert completed, table: " << tableName << ", rows: " << insertedCount);
            return true;
        }
        else
        {
            DBAGENT_LOG_ERROR("commit failed for batch insert, table: " << tableName);
            return false;
        }
    }
    else
    {
        DBAGENT_LOG_WARN("batch insert failed, rolling back, table: " << tableName << ", inserted before failure: " << insertedCount);
        rollback();
        return false;
    }
}
...
std::string SqliteDatabaseWrapper::createWhereCondition(const ListsOfWhereCondition& arguments) const
{
    ...
    for (const auto& [name, group] : groupedWhereConditions)
    {
        ...
        for (const auto& [condition, conditionIndex] : group)
        {
            ...
            {
                DBAGENT_LOG_WARN("Invalid operator in fetchFromDatabase");
                return "";
            }
...
{
    DBAGENT_LOG_DEBUG("fetch data from table: " << tableName << ", from: "
              << location.file_name() << '('
              << location.line() << ':'
              << location.column() << ") `"
              << location.function_name());
    ...
    {
        DBAGENT_LOG_WARN("prepare statment failed, table: " << tableName);
        func({});
        return;
    }
    ...
    for(size_t i = 0; i < arguments.size(); ++i)
    {
        ...
        {
            DBAGENT_LOG_WARN("bind statment failed, table: " << tableName << ", index: " << i);
            func({});
            return;
        }
    }
    ...
    if (auto result = sqlite3_finalize(statement); SQLITE_OK != result && SQLITE_DONE != result)
    {
        DBAGENT_LOG_WARN("finalize statment failed, table: " << tableName << ", error code: " << result);
        func({});
        return;
    }

    DBAGENT_LOG_DEBUG("fetch data succeed, table: " << tableName);
    func(result);
}
...
{
    DBAGENT_LOG_DEBUG("update table: " << tableName << ", from: "
              << location.file_name() << '('
              << location.line() << ':'
              << location.column() << ") `"
              << location.function_name());
    ...
    {
        DBAGENT_LOG_WARN("update with empty data, table: " << tableName);
        return -1;
    }

    if (columns.size() != values.size())
    {
        DBAGENT_LOG_WARN("mismatch in columns and values, table: " << tableName);
        return -1;
    }
    ...
    for (size_t i = 0; i < values.size(); ++i)
    {
        ...
        {
            DBAGENT_LOG_WARN("bind SET value failed, table: " << tableName << ", index: " << i);
            sqlite3_finalize(statement);
            return -1;
        }
    }
    ...
    for (size_t i = 0; i < conditions.size(); ++i)
    {
        ...
        if (op != DBOperatorType::IsNull)
        {
            ...
            {
                DBAGENT_LOG_WARN("bind WHERE value failed, table: " << tableName << ", index: " << i);
                sqlite3_finalize(statement);
                return -1;
            }
        }
    ...
    DBAGENT_LOG_DEBUG("update completed, table: " << tableName << ", rows: " << changedRows);
    return changedRows;
}
...
{
    DBAGENT_LOG_DEBUG("batch update table: " << tableName
              << ", rows: " << items.size()
              << ", from: " << location.file_name() << '(' << location.line() << ')');

    if (keyColumns.empty() || valueColumns.empty() || items.empty())
    {
        DBAGENT_LOG_WARN("batch update with empty data, table: " << tableName);
        return false;
    }
    ...
    for (const auto& item : items)
    {
        ...
        {
            DBAGENT_LOG_ERROR("mismatch in item size, expected: " << expectedSize << ", got: " << item.size() << ", table: " << tableName);
            return false;
        }
    }
    ...
    if (!beginTransaction())
    {
        DBAGENT_LOG_ERROR("failed to begin transaction for batch update, table: " << tableName);
        return false;
    }
    ...
    if (!mDataPrivate->prepareStatement(updateStatement, &statement))
    {
        DBAGENT_LOG_ERROR("prepare statement failed for batch update, table: " << tableName);
        rollback();
        return false;
    }
    ...
    for (const auto& item : items)
    {
        ...
        for (size_t i = 0; i < valueColumns.size(); ++i)
        {
            ...
            {
                DBAGENT_LOG_ERROR("bind SET value failed at row " << updatedCount << ", table: " << tableName);
                success = false;
                break;
            }
        }
        ...
        for (size_t i = 0; i < keyColumns.size(); ++i)
        {
            ...
            {
                DBAGENT_LOG_ERROR("bind WHERE value failed at row " << updatedCount << ", table: " << tableName);
                success = false;
                break;
            }
        }
        ...
        if (auto result = sqlite3_step(statement); SQLITE_OK != result && SQLITE_DONE != result)
        {
            DBAGENT_LOG_ERROR("step failed at row " << updatedCount << ", table: " << tableName << ", error: " << result);
            success = false;
            break;
        }
        ...
        if (auto result = sqlite3_reset(statement); SQLITE_OK != result)
        {
            DBAGENT_LOG_ERROR("reset failed at row " << updatedCount << ", table: " << tableName << ", error: " << result);
            success = false;
            break;
        }
    ...
    if (success)
    {
        if (commit())
        {
            DBAGENT_LOG_INFO("batch update completed, table: " << tableName << ", rows: " << updatedCount);
            return true;
        }
        else
        {
            DBAGENT_LOG_ERROR("commit failed for batch update, table: " << tableName);
            return false;
        }
    }
    else
    {
        DBAGENT_LOG_WARN("batch update failed, rolling back, table: " << tableName << ", updated before failure: " << updatedCount);
        rollback();
        return false;
    }
}
...
{
    DBAGENT_LOG_DEBUG("delete from table: " << tableName << ", from: "
              << location.file_name() << '('
              << location.line() << ':'
              << location.column() << ") `"
              << location.function_name());
    ...
    DBAGENT_LOG_DEBUG("delete completed, table: " << tableName << ", rows: " << deletedRows);
    return deletedRows;
}

bool SqliteDatabaseWrapper::exists(const std::string& tableName, const ListsOfWhereCondition& conditions)
{
    DBAGENT_LOG_DEBUG("check exists in table: " << tableName);
    ...
    DBAGENT_LOG_DEBUG("exists check completed, table: " << tableName << ", found: " << found);
    return found;
}

int64_t SqliteDatabaseWrapper::count(const std::string& tableName, const ListsOfWhereCondition& conditions)
{
    DBAGENT_LOG_DEBUG("count in table: " << tableName);
    ...
    DBAGENT_LOG_DEBUG("count completed, table: " << tableName << ", count: " << rowCount);
    return rowCount;
}
/////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////
