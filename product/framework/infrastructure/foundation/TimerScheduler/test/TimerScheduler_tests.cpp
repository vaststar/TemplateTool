#include <catch2/catch_test_macros.hpp>

#include <ucf/Infrastructure/TimerScheduler/ITimerScheduler.h>
#include <ucf/Infrastructure/TimerScheduler/TimerSchedulerFactory.h>

#include <atomic>
#include <chrono>
#include <thread>

TEST_CASE("scheduleOnce fires once after delay", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    std::atomic<int> n{0};

    auto h = sched->scheduleOnce(std::chrono::milliseconds(50), [&]() { ++n; });
    REQUIRE(h.isValid());

    std::this_thread::sleep_for(std::chrono::milliseconds(200));
    REQUIRE(n.load() == 1);
}

TEST_CASE("scheduleOnce can be cancelled before firing", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    std::atomic<int> n{0};

    auto h = sched->scheduleOnce(std::chrono::milliseconds(200), [&]() { ++n; });
    std::this_thread::sleep_for(std::chrono::milliseconds(50));
    REQUIRE(sched->cancel(h));
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    REQUIRE(n.load() == 0);
}

TEST_CASE("scheduleAtFixedRate fires repeatedly", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    std::atomic<int> n{0};

    sched->scheduleAtFixedRate(std::chrono::milliseconds(0), std::chrono::milliseconds(50), [&]() { ++n; });
    std::this_thread::sleep_for(std::chrono::milliseconds(280));

    const int count = n.load();
    REQUIRE(count >= 3);
    REQUIRE(count <= 8);
}

TEST_CASE("scheduleAtFixedRate stops after cancel", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    std::atomic<int> n{0};

    auto h = sched->scheduleAtFixedRate(std::chrono::milliseconds(0), std::chrono::milliseconds(30), [&]() { ++n; });
    std::this_thread::sleep_for(std::chrono::milliseconds(120));
    REQUIRE(sched->cancel(h));

    const int snapshot = n.load();
    std::this_thread::sleep_for(std::chrono::milliseconds(150));
    REQUIRE(n.load() == snapshot);
}

TEST_CASE("scheduleWithFixedDelay waits for callback completion", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    std::atomic<int> n{0};

    sched->scheduleWithFixedDelay(std::chrono::milliseconds(0), std::chrono::milliseconds(50), [&]() {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        ++n;
    });

    // Each cycle ~= 100ms run + 50ms gap = 150ms. In 400ms expect ~2-3.
    std::this_thread::sleep_for(std::chrono::milliseconds(400));
    const int count = n.load();
    REQUIRE(count >= 1);
    REQUIRE(count <= 4);
}

TEST_CASE("callback can cancel its own handle", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    std::atomic<int> n{0};
    ucf::infrastructure::scheduling::TimerHandle handle;

    handle = sched->scheduleAtFixedRate(std::chrono::milliseconds(0), std::chrono::milliseconds(30), [&]() {
        if (++n == 2)
        {
            sched->cancel(handle);
        }
    });

    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    REQUIRE(n.load() == 2);
}

TEST_CASE("multiple timers fire independently", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    std::atomic<int> a{0};
    std::atomic<int> b{0};

    sched->scheduleAtFixedRate(std::chrono::milliseconds(0), std::chrono::milliseconds(30), [&]() { ++a; });
    sched->scheduleAtFixedRate(std::chrono::milliseconds(0), std::chrono::milliseconds(50), [&]() { ++b; });

    std::this_thread::sleep_for(std::chrono::milliseconds(260));
    REQUIRE(a.load() >= 5);
    REQUIRE(b.load() >= 3);
}

TEST_CASE("FixedRate skips missed ticks instead of bursting", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    std::atomic<int> n{0};

    sched->scheduleAtFixedRate(std::chrono::milliseconds(0), std::chrono::milliseconds(30), [&]() {
        const int seq = ++n;
        if (seq == 1)
        {
            std::this_thread::sleep_for(std::chrono::milliseconds(200));
        }
    });

    // First call blocks 200ms (would miss ~6 ticks). With catch-up: total ~7.
    // With skip-policy: ~1 (initial) + ~5 (remaining 150ms / 30ms) = ~6.
    // The key assertion is "no large burst" -> bounded upper limit.
    std::this_thread::sleep_for(std::chrono::milliseconds(400));
    const int count = n.load();
    REQUIRE(count >= 2);
    REQUIRE(count <= 9);
}

TEST_CASE("shutdown stops further callbacks", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    std::atomic<int> n{0};

    sched->scheduleAtFixedRate(std::chrono::milliseconds(0), std::chrono::milliseconds(30), [&]() { ++n; });
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    sched->shutdown();

    const int snapshot = n.load();
    std::this_thread::sleep_for(std::chrono::milliseconds(150));
    REQUIRE(n.load() == snapshot);
}

TEST_CASE("destructor stops the worker cleanly", "[TimerScheduler]")
{
    std::atomic<int> n{0};
    {
        auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
        sched->scheduleAtFixedRate(std::chrono::milliseconds(0), std::chrono::milliseconds(20), [&]() { ++n; });
        std::this_thread::sleep_for(std::chrono::milliseconds(80));
    }

    const int snapshot = n.load();
    std::this_thread::sleep_for(std::chrono::milliseconds(80));
    REQUIRE(n.load() == snapshot);
}

TEST_CASE("empty callback returns invalid handle", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    auto h = sched->scheduleOnce(std::chrono::milliseconds(10), ucf::infrastructure::scheduling::TimerCallback{});
    REQUIRE_FALSE(h.isValid());
}

TEST_CASE("cancel on invalid handle returns false", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    REQUIRE_FALSE(sched->cancel(ucf::infrastructure::scheduling::TimerHandle{}));
}

TEST_CASE("periodic schedule with zero period returns invalid handle", "[TimerScheduler]")
{
    auto sched = ucf::infrastructure::scheduling::TimerSchedulerFactory::create();
    auto h = sched->scheduleAtFixedRate(std::chrono::milliseconds(0), std::chrono::milliseconds(0), []() {});
    REQUIRE_FALSE(h.isValid());
}
