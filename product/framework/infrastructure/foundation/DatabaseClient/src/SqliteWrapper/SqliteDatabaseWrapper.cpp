...
bool SqliteDatabaseWrapper::DataPrivate::execute(const std::string& commandStr)
{
    ...
    if (!mDatabase)
    {
        DBCLIENT_LOG_ERROR("execute failed: database not open");
        return false;
    }
...
bool SqliteDatabaseWrapper::DataPrivate::prepareStatement(const std::string& statement, sqlite3_stmt** ppStmt)
{
    ...
    if (!mDatabase)
    {
        DBCLIENT_LOG_ERROR("prepareStatement failed: database not open");
        return false;
    }
    ...
    if (SQLITE_OK != result)
    {
        DBCLIENT_LOG_ERROR("prepare statement failed, error code: " << result
...
bool SqliteDatabaseWrapper::DataPrivate::bindDBType(sqlite3_stmt* statement, const DatabaseDataValue& value, int index)
{
    ...
    if (value.isNull())
    {
        ...
        {
            DBCLIENT_LOG_ERROR("db bind null failed, error code: " << result);
            return false;
        }
    }
    ...
    if (value.holdsType<DBSupportedTypes::STRING>())
    {
        ...
        {
            DBCLIENT_LOG_WARN("db bind text failed, error code: " << result);
            return false;
        }
    }
    else if(value.holdsType<DBSupportedTypes::INT>())
    {
        ...
        {
            DBCLIENT_LOG_WARN("db bind int failed, error code: " << result);
            return false;
        }
    }
    else if(value.holdsType<DBSupportedTypes::FLOAT>())
    {
        ...
        {
            DBCLIENT_LOG_WARN("db bind float failed, error code: " << result);
            return false;
        }
    }
    else if(value.holdsType<DBSupportedTypes::BLOB>())
    {
        ...
        if (bufferData.empty())
        {
            ...
            {
                DBCLIENT_LOG_WARN("db empty blob failed, error code: " << result);
                return false;
            }
        }
        else
        {
            ...
            {
                DBCLIENT_LOG_WARN("db blob failed, error code: " << result);
                return false;
            }
        }
    }
    DBCLIENT_LOG_WARN("db bind text failed");
    return false;
}
    ...
    while (SQLITE_ROW == sqlite3_step(statement))
    {
        ...
        for (int i = 0; i < numberOfColumns; ++i)
        {
            ...
            case SQLITE_BLOB:
            {
                ...
                {
                    DBCLIENT_LOG_WARN("Blob seems to be invalid when reading back from the DB.");
            ...
            default:
            {
                DBCLIENT_LOG_WARN("Invalid column type found: " << columnType);
...
SqliteDatabaseWrapper::~SqliteDatabaseWrapper()
{
    ...
    {
        DBCLIENT_LOG_ERROR("close db exception: " << e.what());
    }
    catch(...)
    {
...
bool SqliteDatabaseWrapper::insertIntoDatabase(const std::string& tableName, const Columns& columns, const Arguments& arguments, const std::source_location location)
{
    ...
    {
        DBCLIENT_LOG_WARN("insert empty data into table: " << tableName);
        return false;
    }

    if (columns.size() != arguments.size())
    {
        DBCLIENT_LOG_WARN("mismatch in columns and data, table: " << tableName);
        return false;
    }
    ...
    for (size_t i = 0; i < arguments.size(); ++i)
    {
        ...
        {
            DBCLIENT_LOG_WARN("bind statement failed, table: " << tableName);
            success = false;
            break;
        }
    }
...
bool SqliteDatabaseWrapper::insertBatch(const std::string& tableName, const Columns& columns, const ListOfArguments& arguments, const std::source_location location)
{
    ...
    for (const auto& argument : arguments)
    {
        ...
        {
            DBCLIENT_LOG_ERROR("mismatch in columns and data, table: " << tableName);
            return false;
        }
    }
...
std::string SqliteDatabaseWrapper::createWhereCondition(const ListsOfWhereCondition& arguments) const
{
    ...
    for (const auto& [name, group] : groupedWhereConditions)
    {
        ...
        for (const auto& [condition, conditionIndex] : group)
        {
            ...
            {
                DBCLIENT_LOG_WARN("Invalid operator in fetchFromDatabase");
                return "";
            }
...
int64_t SqliteDatabaseWrapper::updateInDatabase(const std::string& tableName, const Columns& columns, const Arguments& values, const ListsOfWhereCondition& conditions, const std::source_location location)
{
    ...
    {
        DBCLIENT_LOG_WARN("update with empty data, table: " << tableName);
        return -1;
    }

    if (columns.size() != values.size())
    {
        DBCLIENT_LOG_WARN("mismatch in columns and values, table: " << tableName);
        return -1;
    }
    ...
    DBCLIENT_LOG_DEBUG("update completed, table: " << tableName << ", rows: " << changedRows);
    return changedRows;
}
...
bool SqliteDatabaseWrapper::updateBatch(const std::string& tableName, const Columns& keyColumns, const Columns& valueColumns, const ListOfArguments& items, const std::source_location location)
{
    ...
    {
        DBCLIENT_LOG_WARN("batch update with empty data, table: " << tableName);
        return false;
    }
    ...
    for (const auto& item : items)
    {
        ...
        {
            DBCLIENT_LOG_ERROR("mismatch in item size, expected: " << expectedSize << ", got: " << item.size() << ", table: " << tableName);
            return false;
        }
    }
...
int64_t SqliteDatabaseWrapper::deleteFromDatabase(const std::string& tableName, const ListsOfWhereCondition& conditions, const std::source_location location)
{
    ...
    DBCLIENT_LOG_DEBUG("delete completed, table: " << tableName << ", rows: " << deletedRows);
    return deletedRows;
}
