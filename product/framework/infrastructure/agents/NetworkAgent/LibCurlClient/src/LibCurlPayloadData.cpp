...
{
public:
    DataPrivate(ucf::agents::network::http::ByteBufferPtr data, ucf::agents::network::http::UploadProgressFunction progressFunc)
        : mData(data)
        , mTotalSize(data->size())
        , mOffset(0)
        , mProgressFunc(progressFunc)
    {}
    ucf::agents::network::http::ByteBufferPtr getData() const { return mData; }
    size_t getSizeLeft() const { return mTotalSize - mOffset;}
    size_t getOffset() const { return mOffset; }
    void setOffset(size_t offset) { mOffset = offset;}
    size_t getTotalSize() const{ return mTotalSize;}
    ucf::agents::network::http::UploadProgressFunction getProgressFunction() const { return mProgressFunc;}
private:
    ucf::agents::network::http::ByteBufferPtr mData;
    size_t mTotalSize;
    size_t mOffset;
    ucf::agents::network::http::UploadProgressFunction mProgressFunc;
};


BufferPayloadData::BufferPayloadData(ucf::agents::network::http::ByteBufferPtr data, ucf::agents::network::http::UploadProgressFunction progressFunc)
...
size_t BufferPayloadData::readData(char* data, size_t size)
{
    if (auto sizeCopy = mDataPrivate->getSizeLeft(); sizeCopy > 0)
    {
        ...
        ucf::agents::network::http::ByteBuffer slice = ucf::agents::network::http::ByteBuffer(startIter, endIter);
...
{
public:
    DataPrivate(const std::string& filePath, ucf::agents::network::http::UploadProgressFunction progressFunc)
        : mFilePath(filePath)
...
FilePayloadData::FilePayloadData(const std::string& filePath, ucf::agents::network::http::UploadProgressFunction progressFunc)
...
size_t FilePayloadData::readData(char* data, size_t size)
{
    if (auto sizeCopy = mDataPrivate->getSizeLeft(); sizeCopy > 0)
    {
        ...
        ucf::agents::network::http::ByteBuffer slice = ucf::agents::network::http::ByteBuffer(sizeCopy, 0);
